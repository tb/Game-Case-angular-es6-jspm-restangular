...
//// Helpers and config data /////

var errorHandler = function errorHandler(title) {
  return function (err) {
...
var path = {
  ...
  html: '**/*.html',
  templates: 'src/**/*.html',
  scss: ['src/**/*.scss', '!src/assets/**/*.scss'],
  themes: 'src/assets/themes/**/*.scss',
...
gulp.task('scss-themes', function () {
  return gulp.src(path.themes)
    ...
    .pipe(changed(path.output, {extension: '.css'}))
    .pipe(sourcemaps.init())
    .pipe(sass(sassOptions)).on('error', errorHandler('Sass Themes'))
    .pipe(minifyCss({
      compatibility: 'ie8'
    }))
    .pipe(sourcemaps.write("."))
...
gulp.task('compile', function (callback) {
  return runSequence(
    ['scss', 'scss-themes', 'html', 'es6', 'move'],
    callback
  );
});
